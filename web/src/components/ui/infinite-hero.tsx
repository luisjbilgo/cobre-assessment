...
export default function InfiniteHero() {
	...
	return (
		<div
			ref={rootRef}
			className="relative h-svh w-full overflow-hidden bg-[#111] text-white"
		>
            {/* Background Gradient to blend navbar */}
            <div className="absolute top-0 left-0 right-0 h-32 bg-gradient-to-b from-[#111] to-transparent z-20 pointer-events-none" />
			...
			<div className="relative z-10 flex h-svh w-full items-center justify-center px-6">
				<div className="text-center max-w-4xl pt-16">
					<h1
						ref={h1Ref}
						className="mx-auto text-[clamp(2.5rem,5vw,4.5rem)] font-display leading-[1.1] tracking-tight text-white mb-6"
					>
						Soy Luis Bilbao, tu próximo Business Architect
					</h1>
                    <p 
                        ref={subtitleRef}
                        className="text-lg md:text-xl font-medium text-cobre-accent mb-8 tracking-wide uppercase max-w-3xl mx-auto"
                    >
                        Data Science + Business Analytics aplicados a pagos transfronterizos
                    </p>

					<p
						ref={pRef}
						className="mx-auto mt-4 max-w-3xl text-base md:text-lg font-light tracking-tight text-gray-300 text-pretty leading-relaxed"
					>
						Este assessment para Cobre demuestra cómo trabajo: analicé 50,000 transacciones cross-border, diagnostiqué $362K en oportunidad anual, y diseñé la estrategia de inversión 2026. Explora mi metodología, código y hallazgos completos.
					</p>
					...
					>
						<Link
                            href="/insights"
							className="group relative overflow-hidden bg-white text-black px-6 py-3 rounded-full font-medium flex items-center gap-2 hover:bg-gray-100 transition-all cursor-pointer"
						>
						...
						<Link
                            href="/assistant"
							className="group relative px-6 py-3 rounded-full font-medium flex items-center gap-2 text-white border border-gray-600 hover:bg-white/5 transition-all cursor-pointer"
						>
							Hablar con la IA <Bot size={16} />
						</Link>
					</div>
				</div>
			</div>
